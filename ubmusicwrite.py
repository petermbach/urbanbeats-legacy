...
def writeMUSICnodeWSUR(f, ID, nodepart, ncount, x, y, parameter_list):
    ...
    f.write("Stormwater Re-use - Use stored water for irrigation or other purpose,"+str(parameter_list[5])+",\n")
    f.write("Stormwater Re-use - Annual Demand (kL/yr) Scaled by Daily: PET,"+str(parameter_list[6])+",{kL/yr}\n")
...
def writeMUSICjunction(f, jname, ncount, x, y):
    ...
    f.write("Node ID,"+str(ncount)+",{Node ID}\n")
    f.write("Coordinates,"+str(x)+":"+str(y)+",{Coordinates}{X:Y}\n")
    f.write("General - Location,"+str(jname)+",\n")
    f.write("General - Notes,,\n")
    f.write("------------------------------------------------------------------------------------\n")
    return True
