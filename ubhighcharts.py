...
def createDataStringFromData(datadict):
    ...
    for i in datadict.keys():
        seriesstring = "{"
        seriesstring += "name: '"+str(i)+"', data: ["
        for j in range(len(datadict[i])):
            datadict[i][j] = str(datadict[i][j])            
        seriesstring += ','.join(datadict[i])
        seriesstring += ']},\n'
        datastrings += seriesstring
    datastrings += ']'
    return datastrings
...
def line_basic(options_root, charttitle, categories, xlabel, ylabel, valueunits, datadict):
    """Basic Line Chart (as seen @ http://www.highcharts.com/demo/line-basic)
        ...
        - valueunits = the string containing the units for the values in the chart
        - datadict = the full dictionary variable containing all the data. Keys are used for series labels
    """
    #Parse string for data series
...
def TODOarea_range_and_line():
    """Area range with line chart (as seen @ http://www.highcharts.com/demo/arearange-line)"""    
...
def bar_basic(options_root, charttitle, categories, axislabel, valueunits, datadict):
    """Basic Bar Chart (Horizontal) (as seen @ http://www.highcharts.com/demo/bar-basic)
        - charttitle = title of the chart    
        - categories = categories for the chart, x-axis labels e.g. ['Jan', 'Feb', 'Mar', etc.] matrix form
        - axislabel = label for the y-axis
        - valueunits = the string containing the units for the values in the chart
        - datadict = the full dictionary variable containing all the data. Keys are used for series labels
    """
    #Parse datastring from input dictionary
...
def column_basic(options_root, charttitle, categories, axislabel, labelformat, valueunits, datadict):
    """Basic Column Chart (as seen @ http://www.highcharts.com/demo/column-basic)
        ...
        - axislabel = y-axis label for column chart
        - valueunits = units for the data
        - datadict = dictionary containing all relevant data
    """
    #Parse datastring from input dictionary
    datastrings = createDataStringFromData(datadict)
    htmlscript = """ 
    ...
    <html>
        ...
        $(function () {
            ...
            xAxis: {
                categories: """+str(categories)+""",
                labels: { rotation: """+str(labelformat[0])+""",  align: '"""+str(labelformat[1])+"""',
                    style: { fontSize: '"""+str(labelformat[2])+"""px', fontFamily: '"""+str(labelformat[3])+"""'  }}
            },
            yAxis: {
                min: 0,
                title: {
            ...
            tooltip: {
                ...
                pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' +
                    '<td style="padding:0"><b>{point.y:.1f} """+str(valueunits)+"""</b></td></tr>',
                footerFormat: '</table>',
                shared: true,
                useHTML: true
            },
...
def TODOcolumn_drilldown():
    """Column with drill-down (as seen @ http://www.highcharts.com/demo/column-drilldown)"""
...
def pie_basic(options_root, charttitle, seriesname, datadict):
    """Basic Pie Chart (as seen @ http://www.highcharts.com/demo/pie-basic)
        ...
        - datadict = actual dictionary containing all the data
    """
    #Create Data String
    datastrings = "series: [{ type: 'pie', name: '"+str(seriesname)+"',\n data: ["
    for i in datadict.keys():
        datastrings += "['"+str(i)+"', "+str(round(datadict[i],2))+"],\n"
    datastrings += "]}]\n"
...
def TODOdonut_chart():
    """Donut Chart (as seen @ http://www.highcharts.com/demo/pie-donut)"""
...
def TODOdual_axes_line_column():
    """Dual Axes Line and Column Chart (as seen @ http://www.highcharts.com/demo/combo-dual-axes)"""
...
def TODOcolumn_line_and_pie():
    """Column Line and Pie Chart Combo (as seen @ http://www.highcharts.com/demo/combo)"""
...
def box_plot(options_root, charttitle, categories, xlabel, ylabel, valueunits, datadict):
    ...
    $(function () {
        ...
        yAxis: {
            title: {
                text: '"""+str(ylabel)+" ["+str(valueunits)+"]"+"""'
            },
        },
        series: [{
            name: '"""+str(ylabel)+"""',
            ...
            tooltip: {
                headerFormat: '<em>"""+str(xlabel)+""" {point.key}</em><br/>'
            }
        }, {
            name: 'Outlier',
            color: Highcharts.getOptions().colors[0],
            type: 'scatter',
            data: """+str(datadict["Outliers"])+""",
            marker: {
                fillColor: 'white',
                lineWidth: 1,
                lineColor: Highcharts.getOptions().colors[0]
            ...
            tooltip: {
                pointFormat: '"""+str(ylabel)+""": {point.y}'
            }
            }]
        });
    });
...
def spiderweb(options_root, charttitle, categories, valueunits, datadict):
	...
	<head>
        ...
        $(function () {
            ...
            tooltip: {
                shared: true,
                pointFormat: '<span style="color:{series.color}">{series.name}: <b>{point.y:,.0f}"""+str(valueunits)+"""</b><br/>'
            },
            legend: {
                align: 'right',
                verticalAlign: 'top',
...
def TODOmaster_detail_plot():
    """Master Detail Plot (as seen @ http://www.highcharts.com/demo/dynamic-master-detail)"""
